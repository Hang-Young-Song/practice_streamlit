...
def update_price_info(current_price, current_time, stock_code):
    ...
    if (time_key, stock_code) not in price_info_dict:
        price_info_dict[(time_key, stock_code)] = PriceInfo()
        price_info_dict[(time_key, stock_code)].open = current_price
        price_info_dict[(time_key, stock_code)].high = current_price
        price_info_dict[(time_key, stock_code)].low = current_price
        ...
        if current_price > price_info_dict[(time_key, stock_code)].high:
            price_info_dict[(time_key, stock_code)].high = current_price
        if current_price < price_info_dict[(time_key, stock_code)].low:
            price_info_dict[(time_key, stock_code)].low = current_price

        cursor.execute('''
        UPDATE price_info SET high = ?, low = ? WHERE time_key = ? AND stock_code = ?
        ''', (price_info_dict[(time_key, stock_code)].high, price_info_dict[(time_key, stock_code)].low, time_key, stock_code))
        conn.commit()
